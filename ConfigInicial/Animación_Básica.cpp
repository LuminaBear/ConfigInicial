...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(
        WIDTH,
        HEIGHT,
        "Animacion basica - Martinez Perez Isaac Tadeo",
        nullptr,
        nullptr
    );
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // --- CÁLCULO DE TRAYECTORIAS Y COLISIÓN ---
        ...
        // Distancia entre ellos en el plano XZ
        float dx = dogX - ballX;
        float dz = dogZ - ballZ;
        float dist = sqrt(dx * dx + dz * dz);

        // Efecto de rebote
        float bumpRadius = 2.0f; // Distancia a la que empiezan a reaccionar
        float bumpAmount = 0.0f;
        if (dist < bumpRadius) {
            // subida y bajada suave
            bumpAmount = (cos(dist / bumpRadius * 3.14159f) + 1.0f) * 0.5f;
        }

        float dogY = 0.0f + (bumpAmount * 1.5f);  // El perro sube
        float ballY = 2.5f - (bumpAmount * 1.5f); // La pelota empieza alta y baja
        ...
        // Rotar al perro para que siempre mire hacia adelante en su recorrido
        model = glm::rotate(model, -(dogAngle + 1.5708f), glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        model = glm::rotate(model, ballAngle * 4.0f, glm::vec3(1.0f, 0.0f, 0.0f));
